// ============================================================================
    ...
    // v5.2 - BUG FIXES: Auto Layout + Enhanced Message Channel Flow
    // ============================================================================

    console.log("🚀 Initializing Professional LWC Schema Builder (v5.2 - Bug Fixes)...");
    ...
    class DataFlowConnectionRenderer {
        render(nodes, allComponents, svgElement) {
            ...
            console.log("🔍 Rendering Data Flow Mode connections (Enhanced Message Channels)...");
            ...
            nodes.forEach(parentNode => {
                ...
                // ============================================================
                // ENHANCED MESSAGE CHANNEL VISUALIZATION
                // Publisher → MessageChannel → Subscriber
                // ============================================================
                df.lmsChannels?.forEach(lms => {
                    console.log(`  📡 LMS Channel: ${lms.channel} (${lms.type})`);

                    // Find the MessageChannel node
                    const channelNode = nodes.find(n => n.label === lms.channel);

                    if (channelNode) {
                        // Draw connection: Component ←→ MessageChannel
                        if (lms.type === 'publish') {
                            // Publisher → MessageChannel
                            const key = `lms-pub-${parentNode.id}-${channelNode.id}`;
                            if (!drawn.has(key)) {
                                drawn.add(key);
                                console.log(`    ✅ PUBLISHER → MessageChannel: ${parentNode.label} → ${channelNode.label}`);
                                connections.push({
                                    from: parentNode,
                                    to: channelNode,
                                    type: 'message',
                        ...
                        } else if (lms.type === 'subscribe') {
                            // MessageChannel → Subscriber
                            const key = `lms-sub-${channelNode.id}-${parentNode.id}`;
                            if (!drawn.has(key)) {
                                drawn.add(key);
                                console.log(`    ✅ MessageChannel → SUBSCRIBER: ${channelNode.label} → ${parentNode.label}`);
                                connections.push({
                                    from: channelNode,
                                    to: parentNode,
                                    type: 'message',
                        ...
                        // Fallback: Component-to-Component when MessageChannel node not on canvas
                        console.log(`    ⚠️  MessageChannel "${lms.channel}" not on canvas, using fallback`);
                        ...
                        nodes.forEach(otherNode => {
                            ...
                            if (hasMatchingChannel) {
                                const key = `lms-fallback-${parentNode.id}-${otherNode.id}-${lms.channel}`;
                                if (!drawn.has(key)) {
                                    drawn.add(key);
                                    const fromNode = lms.type === 'publish' ? parentNode : otherNode;
                                    const toNode = lms.type === 'publish' ? otherNode : parentNode;

                                    console.log(`    ✅ FALLBACK: ${fromNode.label} → ${toNode.label} (via ${lms.channel})`);
                                    connections.push({
                                        from: fromNode,
                                        to: toNode,
                                        type: 'message',
                                        label: `via ${lms.channel}`
                                    });
                                }
                            }
                        });
        ...
        getLayoutConnections(nodes, allComponents) {
            ...
            nodes.forEach(parentNode => {
                ...
                // Add LMS connections for layout
                parentComp.dataFlow.lmsChannels?.forEach(lms => {
                    // Connect to MessageChannel if present
                    const channelNode = nodes.find(n => n.label === lms.channel);
                    if (channelNode) {
                        if (lms.type === 'publish') {
                            connections.push({ from: parentNode.label, to: lms.channel });
                        } else {
                            connections.push({ from: lms.channel, to: parentNode.label });
                        }
                    }
                });
            });
    ...
    class LWCSchemaBuilder {
        ...
        init() {
            ...
            this.updateUI();
            console.log("✅ LWC Schema Builder initialized (v5.2 - Bug Fixes)");
        }
        ...
        <div class="node-card">
            <div class="node-header">
                <div class="node-title-section">
                    ...
                    <div class="node-type">${comp?.type === 'lmc' ? 'Message Channel' : 'LWC Component'}</div>
                </div>
                <div class="node-header-actions">
                    <button class="node-action-btn node-open-editor" data-component="${node.label}" title="Open in Lightning Studio">
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
                            <path d="M14 2H2a1 1 0 00-1 1v10a1 1 0 001 1h12a1 1 0 001-1V3a1 1 0 00-1-1zM2 3h12v10H2V3zm2 2h8v1H4V5zm0 2h8v1H4V7zm0 2h5v1H4V9z"/>
                        </svg>
                    </button>
                    <button class="node-action-btn node-close-btn" title="Remove from canvas">
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
    ...
    `;
                ...
                el.querySelector('.node-open-editor')?.addEventListener('click', (e) => {
                    e.stopPropagation();
                    this.openInLightningStudio(node.label);
                });
        ...
        handleNodeMouseDown(e, el) {
            if (e.target.closest('.node-close-btn, .node-open-editor')) return;
        ...
        // ========================================================================
        // AUTO LAYOUT - FIXED VERSION
        // ========================================================================

        applyAutoLayout() {
            // FIXED: Always check for components to add, regardless of current nodes
            const componentsToAdd = this.availableComponents.filter(c => 
                !this.nodes.some(n => n.label === c.name)
            );
            
            // Add missing components at origin (they'll be positioned by force layout)
        ...
        openInLightningStudio(componentName) {
            const comp = this.allComponents.find(c => c.name === componentName);
            if (!comp || !comp.id) {
                console.error('Component not found or missing ID:', componentName);
                return;
            }

            this.showEditorModal(comp);
        }

        showEditorModal(comp) {
            const componentPath = `force-app/main/default/lwc/${comp.name}`;
            ...
            modal.innerHTML = `
                <div class="editor-modal-content">
                    <div class="editor-modal-header">
                        <h3>Open Component: ${comp.name}</h3>
                        <button class="editor-modal-close" id="closeEditorModal">×</button>
                    </div>
                    <div class="editor-modal-body">
                        <p class="editor-path">Path: <code>${componentPath}</code></p>
                        
                        <button class="editor-option-btn" id="openVSCodeBtn">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M23.15 2.587L18.21.21a1.494 1.494 0 0 0-1.705.29l-9.46 8.63-4.12-3.128a.999.999 0 0 0-1.276.057L.327 7.261A1 1 0 0 0 .326 8.74L3.899 12 .326 15.26a1 1 0 0 0 .001 1.479L1.65 17.94a.999.999 0 0 0 1.276.057l4.12-3.128 9.46 8.63a1.492 1.492 0 0 0 1.704.29l4.942-2.377A1.5 1.5 0 0 0 24 20.06V3.939a1.5 1.5 0 0 0-.85-1.352zm-5.146 14.861L10.826 12l7.178-5.448v10.896z"/>
                            </svg>
                            Open in VS Code
                            <span class="editor-hint">Will open the component file directly</span>
                        </button>
                        
                        <button class="editor-option-btn" id="openSetupBtn">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                        ...
                        <button class="editor-option-btn editor-copy-btn" id="copyNameBtn">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
        ...
        tryOpenVSCode(componentName, componentPath) {
            const vsCodeUri = `vscode://file${componentPath}/${componentName}.js`;
            
            navigator.clipboard.writeText(componentName).then(() => {
                console.log('✅ Component name copied:', componentName);
            }).catch(() => {
                console.warn('Failed to copy to clipboard');
            });
            
            const uris = [
                `vscode://file/${componentPath}/${componentName}.js`,
                `vscode://file/${componentPath}`,
                `vscode://open?url=file://${componentPath}/${componentName}.js`
            ];
            ...
            setTimeout(() => {
                const message = 
                    ...
                    `3. Type: ${componentName}\n` +
                    `4. Select the file from the list\n\n` +
                    `(Component name has been copied to clipboard)`;
        ...
        copyComponentName(componentName) {
            navigator.clipboard.writeText(componentName).then(() => {
                this.showToast(`✅ Copied: ${componentName}\n\nPress Ctrl+P in VS Code and paste!`, 'success');
                document.getElementById('editorModal')?.remove();
            }).catch(() => {
        ...
        openInSalesforceSetup(componentId) {
            const cached = localStorage.getItem('lwc_schema_cached_data');
            const orgUrl = cached ? JSON.parse(cached).payload?.origin : null;
            
            if (orgUrl) {
                window.open(`${orgUrl}/lightning/setup/LightningComponentBundles/page?address=%2F${componentId}`, '_blank');
                document.getElementById('editorModal')?.remove();
            } else {
                this.showToast('Unable to determine Salesforce org URL', 'error');
            }
        }
    ...
    console.log("✅ Professional LWC Schema Builder loaded (v5.2 - Bug Fixes)");